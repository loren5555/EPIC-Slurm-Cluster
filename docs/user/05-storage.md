...
| `/data`      | 规划中的跨节点共享数据入口       | 当前尚未上线，不要作为作业路径         |

<span style="color:red">
`/workspace` 由 4 块数据盘组成 RAID0 阵列，系统盘不属于该阵列。磁盘吞吐大，但不提供冗余；任意一块数据盘损坏都可能造成整个阵列的数据无法恢复。
...
`/workspace` 是 RAID0 且没有备份，缓存和环境都属于可再生成内容。重要代码、数据和实验结果不要只保存在缓存目录或 `/workspace` 中。规划中的 `/data` 上线前，请使用实验室认可的独立存储保存备份；上线通知发布前不要自行创建或依赖 `/data`。
