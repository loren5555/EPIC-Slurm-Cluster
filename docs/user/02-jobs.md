...
如果程序依赖多个脚本或配置文件，可以把它们放进同一个任务目录。程序代码和大型数据不要复制到 Job Composer 目录中；当前脚本应通过绝对路径或工作目录访问目标计算节点的 `/workspace`。规划中的共享 `/data` 尚未上线。
...
输出应写入所选计算节点的 `/workspace`，并把重要结果复制到独立存储。规划中的共享 `/data` 正式上线前不要依赖该路径。
